...
export function CartView() {
  ...
  return <main className="cart-page page-shell"><header className="route-header"><p className="eyebrow">SELECTED OBJECTS</p><h1>YOUR BAG <span>({cartCount})</span></h1></header>
    {cartProducts.length === 0 ? <div className="empty-bag glass-panel"><p>Nothing is asking to be seen yet.</p><Link href="/shop" className="editorial-link"><TextShuffle text="enter the shop" /> <span>↗</span></Link></div> : <div className="cart-layout">
      <div className="cart-lines">{cartProducts.map(({ product, quantity }) => <article className="cart-line" key={product.id}>
        ...
        <div className="quantity-control" aria-label={`Quantity for ${product.name}`}><button onClick={() => setQuantity(product.id, quantity - 1)} aria-label={`Decrease ${product.name} quantity`}>−</button><span>{quantity}</span><button disabled={quantity >= product.inventory} onClick={() => setQuantity(product.id, quantity + 1)} aria-label={`Increase ${product.name} quantity`} title={quantity >= product.inventory ? "Maximum available quantity in your bag" : undefined}>+</button></div>
